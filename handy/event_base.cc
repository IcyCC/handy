...
struct EventsImp {
    ...
    void wakeup() {
        ...
        handy_fatalif_log(r <= 0, "write error wd %d %d %s", r, errno, strerror(errno));
    }
...
void EventsImp::init() {
    ...
    handy_fatalif_log(r, "pipe failed %d %s", errno, strerror(errno));
    r = util::addFdFlag(wakeupFds_[0], FD_CLOEXEC);
    handy_fatalif_log(r, "addFdFlag failed %d %s", errno, strerror(errno));
    r = util::addFdFlag(wakeupFds_[1], FD_CLOEXEC);
    handy_fatalif_log(r, "addFdFlag failed %d %s", errno, strerror(errno));
    handy_trace_log("wakeup pipe created %d %d", wakeupFds_[0], wakeupFds_[1]);
    ...
    ch->onRead([=] {
        ...
        if (r > 0) {
            ...
            handy_fatal_log("wakeup channel read error %d %d %s", r, errno, strerror(errno));
        }
    });
}
...
Channel::Channel(EventBase *base, int fd, int events) : base_(base), fd_(fd), events_(events) {
    handy_fatalif_log(net::setNonBlock(fd_) < 0, "channel set non block failed");
...
void Channel::close() {
    if (fd_ >= 0) {
        handy_trace_log("close channel %ld fd %d", (long) id_, fd_);
...
TcpConn::~TcpConn() {
    handy_trace_log("tcp destroyed %s - %s", local_.toString().c_str(), peer_.toString().c_str());
    delete channel_;
}
...
void TcpConn::reconnect() {
    ...
    handy_info_log("reconnect interval: %d will reconnect after %lld ms", reconnectInterval_, interval);
