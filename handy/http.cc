        ...
        *line1 = req.eatLine();
        while (req.size()) {
            ...
            if (k.size() && ln.size() && k.back() == ':') {
                ...
                handy_error_log("bad http line: %.*s %.*s", (int) k.size(), k.data(), (int) ln.size(), ln.data());
                return Error;
            }
        }
        scanned_ += 4;
...
HttpMsg::Result HttpRequest::tryDecode(Slice buf, bool copyBody) {
    ...
    if (ln1.size()) {
        ...
        if (query_uri.size() == 0 || query_uri[0] != '/') {
            handy_error_log("query uri '%.*s' should begin with /", (int) query_uri.size(), query_uri.data());
            return Error;
        }
...
void HttpConnPtr::handleRead(const HttpCallBack &cb) const {
    if (!tcp->isClient()) {  // server
        ...
        if (r == HttpMsg::Continue100) {
            ...
            handy_info_log("http request: %s %s %s", req.method.c_str(), req.query_uri.c_str(), req.version.c_str());
            handy_trace_log("http request:\n%.*s", (int) tcp->input_.size(), tcp->input_.data());
            cb(*this);
        }
    } else {
        HttpResponse &resp = getResponse();
        ...
        if (r == HttpMsg::Complete) {
            handy_info_log("http response: %d %s", resp.status, resp.statusWord.c_str());
            handy_trace_log("http response:\n%.*s", (int) tcp->input_.size(), tcp->input_.data());
            cb(tcp);
        }
    }
}
